...
def mpt_analytic(data, mean_returns, cov_matrix, T, number_of_decimals, effecient_frontier_means_range,
    ...
    if np.linalg.cond(cov_matrix) < 10 ** 15:
        ...
        boundary_poly_coefs = np.polynomial.polynomial.polyfit(efficient_frontier_vols, efficient_frontier_means,
                                                               effecient_frontier_polynomial_approximation_degree)

        CML_volatility_range = (0, max(efficient_frontier_vols))
        CML_means, CMl_vars = get_analytic_capital_market_line(w_mk, mean_returns, cov_matrix, T,
        ...
        (market_portfolio_allocation, mk_ret, mk_vol, mk_sharpe, min_vol_allocation,
         min_volatility_ret, min_volatility_vol, min_volatility_sharpe, efficient_frontier_vols,
         efficient_frontier_means, boundary_poly_coefs, CMl_vars, CML_means) = (None, None, None, None, None, None,
                                                                                None, None, None, None, None, None,
                                                                                None)
        singular = True

    return ((market_portfolio_allocation, mk_ret, mk_vol, mk_sharpe),
