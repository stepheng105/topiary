...
from modern_portfolio_theory.empirical_solution_helpers import max_sharpe_ratio, min_variance, efficient_frontier

def mpt_empirically(data, mean_returns, cov_matrix, T, number_of_decimals, tolerance, effecient_frontier_means_range, efficient_frontier_granularity, effecient_frontier_polynomial_approximation_degree):
    ...
    dx = target_means[1] - target_means[0]
    target_means = np.append(target_means, target_means[len(target_means) - 1] + dx)
    target_means = np.append(target_means[0] - dx, target_means)
    efficient_portfolio_vols = efficient_frontier(mean_returns, cov_matrix, target_means, T, tolerance)
    ...
    return ((max_sharpe_allocation, max_sharpe_ret, max_sharpe_vol, max_sharpe_sharpe),
            (min_vol_allocation, min_volatility_ret, min_volatility_vol, min_volatility_sharpe),
