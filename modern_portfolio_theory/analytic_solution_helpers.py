...
# Function for solving the matrix equation Ax = b for x. It will use sci.linalg.solve to get the exact answer so long
# as A is not too close to being singular. If it is, it will switch to using least squares to solve the matrix
# equation
# ------------------------------------------------------------------------------------------------------
# Input: A (numpy matrix): An n x n square matrix b (numpy vector): A length n vector assume_a (string): A constant
# for sci.linalg.solve to assume things about the matrix A (see
# https://docs.scipy.org/doc/scipy/reference/generated/scipy.linalg.solve.html#scipy.linalg.solve) Output: x (numpy
# vector): The vector that solves the equation Ax = b. If A is singular or close to singular, it returns None.
...
# Function that is simply outputs the inverse of the function that takes in lambda and outputs the mean return of the
# optimal portfolio with the given lambda scaler
# ------------------------------------------------------------------------------------------------------
# Inputs:
#   mean_range (tuple): A tuple of floats corresponding to the interval for the mean range
#   mean_returns (numpy array): An array with the mean returns of n different stocks
#   cov_matrix (numpy matrix): The (n)x(n) covariance matrix for the stocks
#   mk_exp_ret (float): The expected return of the market portfolio
#   min_var_exp_ret (float): The expected return of the minimum variance portfolio
# Outputs:
#   lambda_range (tuple): The inverse image of mean_range for the function
#                         F(lambda) = E(P) where P solves m*w - lambda w^T*C*w
...
# Output:
#   analytic_efficient_frontier_volatilities (numpy array): A list of the portfolio weights
