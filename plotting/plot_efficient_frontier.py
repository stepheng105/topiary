...
from modern_portfolio_theory.topiary import topiary, topiary_main_algorithm


def print_portfolio(name, allocation, expected_return, volatility, sharpe_ratio, eta_value, num_decimals, show_0_allocation=False):
    ...
    print("Eta:", round(eta_value, num_decimals))
    print("\n")
...
def plot_efficient_frontier(data, T, num_portfolios_to_simulate, empirical_MPT=True, analytic_MPT=False, num_decimals=2,
                            tolerance=10e-10, efficient_frontier_granularity=50,
                            topiaric_efficient_frontier_granularity=1000, const_for_topiary=None,
                            positive_weights_only_for_topiary=True, too_high_rate=None,
                            starting_weight_for_topiary=None,
                            efficient_frontier_polynomial_approximation_degree=4):
    T = round(T * TIME_PERIOD_IN_YEARS)
    ...
    if const_for_topiary is not None:
        ((topiary_allocation, topiary_expected_return, topiary_volatility, topiary_sharpe_ratio, topiary_eta),
         (min_eta_allocation, min_eta_expected_return, min_eta_volatility, min_eta_sharpe_ratio, min_eta_eta),
         (topiary_efficient_returns, topiary_efficient_vols), boundary_poly_coefs_top, path_taken_info) = topiary(data, mean_returns.copy(), cov_matrix.copy(), T, tolerance,
                                    ...
                                    number_of_decimals=num_decimals, topiaric_efficient_frontier_granularity=topiaric_efficient_frontier_granularity,
                                    efficient_frontier_polynomial_approximation_degree=efficient_frontier_polynomial_approximation_degree)
    ...
    if empirical_MPT:
        ...
        ((max_sharpe_allocation_emp, max_sharpe_ret_emp, max_sharpe_vol_emp, max_sharpe_sharpe_emp, max_sharpe_eta_emp),
            (min_vol_allocation_emp, min_volatility_ret_emp, min_volatility_vol_emp, min_volatility_sharpe_emp, min_volatility_eta_emp),
    ...
    if analytic_MPT:
        ...
        lambda_range = (min(min(results[1]), min(an_rt_individual_stocks)),
                        max(max(results[1]), max(an_rt_individual_stocks)))
        ((market_portfolio_allocation_anal, mk_ret_anal, mk_vol_anal, mk_sharpe_anal, mk_eta_anal),
         (min_vol_allocation_anal, min_volatility_ret_anal, min_volatility_vol_anal, min_volatility_sharpe_anal, min_volatility_eta_anal),
         ...
         (CMl_vars_anal, CML_means_anal), singular) = mpt_analytic(data, mean_returns.copy(), cov_matrix.copy(), T,
                                                                   const_for_topiary, num_decimals,
                                                                   lambda_range,
                                                                   efficient_frontier_granularity,
                                                                   efficient_frontier_polynomial_approximation_degree)
        if singular:
    ...
    plt.scatter(results[0, :], results[1, :], c=results[2, :], cmap='YlGnBu', marker='o', s=10, zorder=-1)
    color_bar = plt.colorbar()
    color_bar.set_label('Aesthetic Objective')
    ...
    if const_for_topiary is not None:
        print_portfolio("Topiaric", topiary_allocation, topiary_expected_return, topiary_volatility,
                        topiary_sharpe_ratio, topiary_eta, num_decimals)
        print_portfolio("SciPy-Topiaric", min_eta_allocation, min_eta_expected_return, min_eta_volatility,
                        min_eta_sharpe_ratio, min_eta_eta, num_decimals)
        print_polynomial_approximation("Empirical", boundary_poly_coefs_top.copy(), num_decimals)
        plt.plot(topiary_efficient_vols, topiary_efficient_returns, linestyle='-.', color='black',
                 label='Topiaric Positive Efficient Frontier', zorder=1)
        # plt.scatter(path_taken_info[0, :], path_taken_info[1, :], c=path_taken_info[2, :], cmap='YlGnBu', marker='o',
        #             s=10)
        # plt.plot(np.polyval(boundary_poly_coefs_top[::-1], topiary_efficient_returns), topiary_efficient_returns, color='yellow', alpha=0.5,
    ...
    if empirical_MPT:
        print_portfolio("Maximum Sharpe Ratio", max_sharpe_allocation_emp, max_sharpe_ret_emp, max_sharpe_vol_emp,
                        max_sharpe_sharpe_emp, max_sharpe_eta_emp, num_decimals)
        print_portfolio("Minimum Volatility", min_vol_allocation_emp, min_volatility_ret_emp, min_volatility_vol_emp,
                        min_volatility_sharpe_emp, min_volatility_eta_emp, num_decimals)
        print_polynomial_approximation("Empirical", boundary_poly_coefs_emp.copy(), num_decimals)
        print('\n')
        plt.plot(efficient_portfolio_means_emp, target_emp, linestyle='-', color='pink',
                 label='Positive Efficient Frontier', zorder=0)
    ...
    if analytic_MPT and not singular:
        print_portfolio("Analytic Market", market_portfolio_allocation_anal, mk_ret_anal, mk_vol_anal, mk_sharpe_anal,
                        mk_eta_anal, num_decimals)
        print_portfolio("Analytic Minimum Volatility", min_vol_allocation_anal, min_volatility_ret_anal,
                        min_volatility_vol_anal, min_volatility_sharpe_anal, min_volatility_eta_anal, num_decimals)
    ...
    if const_for_topiary is not None:
        ...
        plt.scatter(topiary_volatility, topiary_expected_return, marker='*', color='cyan', s=75,
                    label='Topiaric portfolio', zorder=4)
        # plt.plot(np.polyval(boundary_poly_coefs_top[::-1], topiary_efficient_returns), topiary_efficient_returns,
        #          color='yellow', alpha=0.5,
        #          label='polynomial approximation', zorder=1)

        plt.title('Calculated Portfolio Optimization based on Efficient Frontier using topiary algorithm')
        plt.xlabel('annualised volatility')
        plt.ylabel('annualised returns')
        plt.legend(labelspacing=0.8)
        plt.savefig('Topiary.png', dpi=300)
    ...
    if empirical_MPT:
        ...
        plt.title('Calculated Portfolio Optimization based on Efficient Frontier using scipy')
        plt.xlabel('annualised volatility')
        plt.ylabel('annualised returns')
        plt.legend(labelspacing=0.8)
        plt.savefig('SciPy.png', dpi=300)